...
public class LocalTimeFormatter extends BaseTypeHandler<LocalTime> {
	
	@Override
	public void setNonNullParameter(PreparedStatement ps, int i, LocalTime ldt, JdbcType jdbcType)
			throws SQLException {
		
		ps.setString(i,ldt.format(DateTimeUtility.DB_TIME_FORMATTER) );
	}
	...
	private LocalTime parse(String str) {
		...
		return LocalTime.parse(str, DateTimeUtility.DB_TIME_FORMATTER);
	}
